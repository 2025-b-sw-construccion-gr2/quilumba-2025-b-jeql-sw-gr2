...
public class ControlVeterinario extends HttpServlet {
    ...
    private void ruteador(HttpServletRequest req, HttpServletResponse resp)
        ...
        String accion =
                req.getParameter("accion") != null ? req.getParameter("accion") : "ingresarModulo";
    ...
    private void ingresarModulo(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
    ...
    private void iniciarEdicion(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
    ...
    private void desactivarVeterinario(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (cedula != null && !cedula.isEmpty()) {
            ...
            if (resultado) {
                ...
                req.getSession()
                        .setAttribute(
                                "mensaje", "Error: El veterinario no existe o ya está inactivo");
            }
        }
        resp.sendRedirect("ControlVeterinario?accion=ingresarModulo");
    }
    ...
    private void iniciarDesactivacion(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (veterinario != null) {
            req.setAttribute(
                    "mensajeConfirmacion",
                    "¿Está seguro que desea desactivar al veterinario "
    ...
    private boolean datosIncompletos(Veterinario v) {
        return v.getNombre() == null
                || v.getNombre().trim().isEmpty()
                || v.getApellido() == null
                || v.getApellido().trim().isEmpty()
                || v.getEspecialidad() == null
                || v.getEspecialidad().trim().isEmpty()
                || v.getUsuario() == null
                || v.getUsuario().trim().isEmpty()
                || v.getNumeroLicencia() == null
                || v.getNumeroLicencia().trim().isEmpty();
    }
}
