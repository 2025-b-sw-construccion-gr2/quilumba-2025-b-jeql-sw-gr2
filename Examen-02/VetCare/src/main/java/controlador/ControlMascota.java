...
public class ControlMascota extends HttpServlet {
    ...
    private void ingresarModulo(
            HttpServletRequest req, HttpServletResponse resp, String cedulaCliente)
            throws ServletException, IOException {
        req.setAttribute(
                "mascotas",
                FactoryDAO.getFactory().getMascotaDAO().obtenerPorCliente(cedulaCliente));
    ...
    private void registrarMascota(HttpServletRequest req, HttpServletResponse resp, Cliente cliente)
        ...
        try {
            ...
            if (pesoStr != null && !pesoStr.isEmpty()) mascota.setPeso(Double.parseDouble(pesoStr));
    ...
    private void iniciarEdicion(
        ...
        if (idStr != null && !idStr.isEmpty()) {
            Mascota mascota =
                    FactoryDAO.getFactory().getMascotaDAO().obtenerPorId(Integer.parseInt(idStr));
    ...
    private void editarMascota(
        ...
        if (idStr != null && !idStr.isEmpty()) {
            ...
            if (mascota != null && mascota.getCliente().getCedula().equals(cedulaCliente)) {
                ...
                req.getSession()
                        .setAttribute("mensaje", "No tiene permiso para editar esta mascota");
            }
        }
        resp.sendRedirect("ControlMascota?accion=ingresarModulo");
    }
    ...
    private void eliminarMascota(
        ...
        if (idStr != null && !idStr.isEmpty()) {
            ...
            if (mascota != null && mascota.getCliente().getCedula().equals(cedulaCliente)) {
                ...
                req.getSession()
                        .setAttribute("mensaje", "No tiene permiso para eliminar esta mascota");
            }
        }
        resp.sendRedirect("ControlMascota?accion=ingresarModulo");
    }
    ...
    private boolean datosIncompletos(Mascota m) {
        return m.getNombre() == null
                || m.getNombre().trim().isEmpty()
                || m.getEspecie() == null
                || m.getEspecie().trim().isEmpty();
    }
}
