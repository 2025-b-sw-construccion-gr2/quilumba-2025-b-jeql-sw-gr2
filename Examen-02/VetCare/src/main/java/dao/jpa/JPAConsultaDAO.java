...
public class JPAConsultaDAO extends JPAGenericDAO<Consulta, Integer> implements ConsultaDAO {
    ...
    public List<Consulta> obtenerPorUsuario(modelo.Usuario usuario) {
        try {
            ...
            if (usuario instanceof modelo.Veterinario) {
                ...
                jpql =
                        "SELECT c FROM Consulta c LEFT JOIN FETCH c.mascota m LEFT JOIN FETCH m.cliente WHERE c.veterinario.cedula = :cedula OR c.veterinario IS NULL ORDER BY c.fecha, c.hora";
                ...
                jpql =
                        "SELECT c FROM Consulta c JOIN c.mascota m WHERE m.cliente.cedula = :cedula ORDER BY c.fecha, c.hora";
                return em.createQuery(jpql, Consulta.class)
                        .setParameter("cedula", usuario.getCedula())
                        .getResultList();
                ...
                jpql =
                        "SELECT c FROM Consulta c LEFT JOIN FETCH c.mascota m LEFT JOIN FETCH m.cliente LEFT JOIN FETCH c.veterinario ORDER BY c.fecha, c.hora";
    ...
    @Override
    public List<java.sql.Time> obtenerConsultasDisponibles(
            java.util.Date fecha, String idVeterinario) {
        try {
